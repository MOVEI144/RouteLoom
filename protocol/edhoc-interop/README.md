...
Recorded EDHOC method 0 / cipher suite 2 handshakes between the
...
the SDK v1 join profile plus the P4 member handshake (design-p4 §5).
...
| [method0_member.txt](method0_member.txt) | session State items (label `-65543`); Intent in EAD_1, ContextConfirm in EAD_4, five session Exporter outputs (§5.4) | 832 B / 856 B |
...
| check | where | profile |
|---|---|---|
| Rust reproduces its own messages from the recorded inputs and accepts libedhoc's | `cargo test -p routeloom-edhoc --test interop` (`transcript_replays_byte_for_byte`, always) | both join files |
| Rust reproduces the member handshake and the five session Exporter outputs | `cargo test -p routeloom-edhoc --test member` (`transcript_replays_byte_for_byte`, always) | member file |
...
| libedhoc replays the member handshake incl. the five Exporter outputs | ctest `routeloom_edhoc_interop_replay_member` | member file |
...
| live member handshake over pipes, compared with the file | `ROUTELOOM_EDHOC_PEER=<build>/tests/cpp/routeloom_edhoc_interop_peer cargo test -p routeloom-edhoc --test member live_against_libedhoc -- --nocapture` | member file |
