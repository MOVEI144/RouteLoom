//! SHA-256 (FIPS 180-4), std-only — the same portable implementation the
//! workspace already carries in `routeloom-host/src/canonical.rs` (which
//! cannot be imported from a binary crate). Identical output to the
//! device's `Sha256`/`sha256` in `discovery_scope.cpp`: this is the hash
...
mod tests {
    ...
    #[test]
    fn sha256_matches_classic_vectors() {
        assert_eq!(
        ...
        // Two-block input crossing the padding boundary.
        let m = [0x61u8; 64];
        assert_eq!(
            hex_lower(&sha256(&m)),
