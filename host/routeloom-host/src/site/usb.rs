...
/// True for the HostOps bodies this lane owns (0x60-0x63; the frame
/// router's test — the bodies only reach it session-verified).
...
/// Verified 0x60-0x63 inner bodies posted by the USB read thread, FIFO so
/// ups, aborts and results are applied in wire order. Never blocks the
/// reader: a full inbox drops the body (the device resends inside its
/// own ack window, or the authority timers end the attempt).
#[derive(Default)]
...
/// Lane-private state: the bound session and its adapter.
#[derive(Default)]
pub struct SiteLane {
