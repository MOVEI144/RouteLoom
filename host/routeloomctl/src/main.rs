...
/// Build the API1 `config.recover` request line — a signed RCR1
/// StoreRecover object issued + transferred on the dedicated recovery
/// lane, reachable while the target journal is quarantined/uncertain.
/// `attest` is omitted when 0 (adopt surviving record); 1 attests
...
fn config_recover_request(
    ...
    let attest_field = if attest == 0 {
        String::new()
    } else {
        format!(",\"attest\":{attest}")
    };
    format!(
        "API1 {{\"v\":1,\"request_id\":\"{}\",\"method\":\"config.recover\",\"params\":{{\"network\":\"{network}\",\"target\":\"{target}\",\"config_namespace\":{ns},\"schema\":{schema},\"new_store_generation\":{new_store_generation}{attest_field}}}}}",
        request_id(),
    )
}
...
/// <u16> --schema <u16> --new-store-generation <u32> [--attest 0|1]`.
/// Issues a signed RCR1 StoreRecover object on the dedicated recovery
/// lane (HostOps 0x24) — reachable while the target journal is
/// quarantined/uncertain. The daemon signs under the current authority
/// generation; the real verdict arrives via `config-get`.
fn config_recover_command(args: &[String]) -> Result<String, Box<dyn std::error::Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--attest" => {
                let raw = opt_value(&mut args, "--attest")?;
                attest = match raw.parse::<u8>() {
                    Ok(n @ (0 | 1)) => n,
                    _ => return Err("--attest must be 0 (adopt) or 1 (reprovision)".into()),
                };
            }
