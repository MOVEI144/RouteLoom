...
namespace {
// Two-partition fake: the migration marker lives in the default "nvs"
// partition (independent of rlsec), the legacy c/f/r records in "rlsec".
// rlsec_full injects a full security partition: its writes refuse with
// NOT_ENOUGH_SPACE while reads (and every default-partition op) work.
struct Entry {
  const char* part;
  const char* space;
  const char* key;
...
std::array<Entry, 8> entries{{
    {"nvs", "rldev", "migration", 0xDEADBEEFU, true, true},
    {"rlsec", "rlcounter", "c00000001", 0, true, true},
    {"rlsec", "rlreplay", "f00000002", 0, true, true},
    {"rlsec", "rlreplay", "r00000003", 0, true, true},
    {"rlsec", "rlcounter", "cmax", 0, true, true},
    {"rlsec", "rlreplay", "f0000000x", 0, true, true},
    {"rlsec", "rldev", "boot_hi", 44, true, true},
    {"rlsec", "rlres", "s00", 0, true, true},
}};
const char* opened_part = nullptr;
const char* opened_space = nullptr;
std::size_t erase_calls = 0;
bool invalid_find = false;
bool rlsec_full = false;
}  // namespace
...
esp_err_t nvs_open(const char* space, int, nvs_handle_t* handle) {
  if (space == nullptr || handle == nullptr) return ESP_ERR_INVALID_ARG;
  opened_part = "nvs";
  opened_space = space;
  *handle = 2;
  return ESP_OK;
...
esp_err_t nvs_get_u32(nvs_handle_t, const char* key, std::uint32_t* value) {
  for (const auto& entry : entries) {
    if (entry.live && std::strcmp(entry.part, opened_part) == 0 &&
        std::strcmp(entry.space, opened_space) == 0 && std::strcmp(entry.key, key) == 0) {
...
esp_err_t nvs_set_u32(nvs_handle_t, const char* key, std::uint32_t value) {
  if (rlsec_full && std::strcmp(opened_part, "rlsec") == 0) {
    return ESP_ERR_NVS_NOT_ENOUGH_SPACE;
  }
  for (auto& entry : entries) {
    if (std::strcmp(entry.part, opened_part) == 0 &&
        std::strcmp(entry.space, opened_space) == 0 && std::strcmp(entry.key, key) == 0) {
      entry.value = value;
      entry.live = true;
      entry.u32 = true;
...
esp_err_t nvs_entry_find(const char* partition, const char* space, int,
  ...
  *iterator = new NvsIterator{partition, space, static_cast<std::size_t>(-1)};
...
esp_err_t nvs_entry_next(nvs_iterator_t* iterator) {
  ...
  const std::size_t start =
      (*iterator)->index == static_cast<std::size_t>(-1) ? 0 : (*iterator)->index + 1;
  for (std::size_t i = start; i < entries.size(); ++i) {
    if (entries[i].live && std::strcmp(entries[i].part, (*iterator)->part) == 0 &&
        std::strcmp(entries[i].space, (*iterator)->space) == 0) {
      (*iterator)->index = i;
      return ESP_OK;
    }
  }
...
int main() {
  ...
  // #37: a full rlsec must not block the migration marker — it commits
  // to the independent default partition — while the legacy erases fail
  // honestly (nothing deleted, survivors still listed).
  entries[0].live = false;  // no marker yet
  rlsec_full = true;
