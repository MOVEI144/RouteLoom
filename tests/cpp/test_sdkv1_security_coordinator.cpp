...
void test_dev_channel_failure_rebuilds_small_side() {
  current = "dev_channel_failure_rebuilds_small_side";
  Fixture f{};
  CHECK(f.init_stores());
  SecurityCoordinator coordinator(f.deps());
  CHECK(coordinator.adopt_dev(dev_config(), kT0).ok());
  // A failed channel apply in Dev parks in Recovery like the member
  // route — with the small side rebuilt, as every non-Dev mode has it.
  CoordinatorEvent failed{};
  failed.kind = CoordinatorEventKind::ChannelReady;
  failed.now = kT0 + 100;
  failed.channel_token = 0;
  failed.channel_result = StatusCode::RadioFailure;
  failed.channel = dev_config().channel;
  CHECK(coordinator.step(failed).ok());
  CHECK(coordinator.snapshot().mode == CoordinatorMode::Recovery);
  CoordinatorAction action{};
  CHECK(coordinator.take_action(action).ok());
  CHECK(action.kind == CoordinatorActionKind::ReportRecovery);
  CHECK(!coordinator.snapshot().authority_started);
  // Staging and the channel view work on the rebuilt side.
  BootstrapMeta meta{};
  meta.origin = kNode + 1;
  const std::uint8_t body[2] = {0x01, 0x04};
  const std::uint32_t staged0 = coordinator.counters().staged_drops;
  CHECK(coordinator.on_frame(meta, FrameType::BootstrapAuth, ByteView{body, 2}, kT0 + 100).ok());
  CHECK(coordinator.counters().staged_drops == staged0);
}
