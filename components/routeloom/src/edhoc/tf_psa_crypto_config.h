 ...
 * block cipher and the CCM mode are compiled: they back the AES-CCM-16-64-128
 * AEAD of EDHOC cipher suite 2 in host builds (components/routeloom/src/edhoc/
 * edhoc_aead_host.cpp). No PSA core, no RNG, no other algorithm is built; the
 * ESP-IDF component does not compile this subset at all (firmware uses the
 * PSA API of ESP-IDF's own Mbed TLS instead).
 *
 * MBEDTLS_PSA_CRYPTO_C is what makes the builtin-driver adjustment headers
 * turn PSA_WANT_KEY_TYPE_AES / PSA_WANT_ALG_CCM into MBEDTLS_AES_C /
 * MBEDTLS_CCM_C; psa_crypto.c itself is not compiled, so
 * MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG only keeps the configuration consistent
 * (no entropy module is pulled in).
 *
