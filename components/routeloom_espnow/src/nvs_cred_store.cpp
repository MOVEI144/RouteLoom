...
Status slot_key(const std::uint8_t slot, char (&key)[4]) noexcept {
  if (slot >= kCredentialSlots) {
    return Status::error(StatusCode::InvalidArgument,
                         "NVS credential slot out of range");
  }
  std::snprintf(key, sizeof(key), "d%u", static_cast<unsigned>(slot));
  return Status::success();
}

}  // namespace

NvsCredStore::~NvsCredStore() { close(); }

Status NvsCredStore::open(const char* name_space) noexcept {
  if (name_space == nullptr || name_space[0] == '\0') {
    return Status::error(StatusCode::InvalidArgument,
                         "NVS credential namespace missing");
  }
  close();
...
void NvsCredStore::close() noexcept {
  if (open_) nvs_close(handle_);
  handle_ = 0;
  open_ = false;
}

Status NvsCredStore::read(const std::uint8_t slot, const MutableByteView target) noexcept {
  if (!open_ || target.data == nullptr || target.size == 0) {
    return Status::error(StatusCode::InvalidState, "NVS credential store not ready");
  }
  char key[4]{};
...
Status NvsCredStore::write(const std::uint8_t slot, const ByteView data) noexcept {
  if (!open_ || data.data == nullptr || data.size == 0 ||
      data.size > kCredentialSlotBytes) {
    return Status::error(StatusCode::InvalidState, "NVS credential store not ready");
  }
  char key[4]{};
  const auto status = slot_key(slot, key);
