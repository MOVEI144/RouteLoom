...
Status slot_key(const std::uint8_t slot, char (&key)[4]) noexcept {
  if (slot >= kTrustStoreSlots) {
    return Status::error(StatusCode::InvalidArgument, "NVS trust slot out of range");
  }
...
NvsTrustStore::~NvsTrustStore() { close(); }

Status NvsTrustStore::open(const char* name_space) noexcept {
  if (name_space == nullptr || name_space[0] == '\0') {
    return Status::error(StatusCode::InvalidArgument, "NVS trust namespace missing");
  }
  close();
...
void NvsTrustStore::close() noexcept {
  if (open_) nvs_close(handle_);
  handle_ = 0;
  open_ = false;
}

Status NvsTrustStore::read(const std::uint8_t slot, const MutableByteView target) noexcept {
  if (!open_ || target.data == nullptr || target.size == 0) {
    return Status::error(StatusCode::InvalidState, "NVS trust store not ready");
  }
  char key[4]{};
  ...
  if (error == ESP_ERR_NVS_NOT_FOUND) {
    // Never written: a uniformly-erased slot the store classifies Empty.
  ...
  if (actual > target.size) {
    ...
    // fixed non-erased pattern so the store quarantines it.
  ...
  // The store wrote only used_len bytes; the unread tail of the slot
  ...
  // Empty; a present-but-erased blob is reported Corrupt so the store
  // quarantines instead of adopting "provably absent" siblings.
...
Status NvsTrustStore::write(const std::uint8_t slot, const ByteView data) noexcept {
  if (!open_ || data.data == nullptr || data.size == 0 ||
      data.size > kTrustStoreSlotBytes) {
    return Status::error(StatusCode::InvalidState, "NVS trust store not ready");
  }
  char key[4]{};
  const auto status = slot_key(slot, key);
  ...
  // Count only committed writes — the §4.9 wear budget measures what
