...
constexpr psa_algorithm_t kGcm = PSA_ALG_GCM;
...
bool import_key(const std::uint8_t* key, psa_key_id_t& key_id) noexcept {
  ...
  psa_set_key_algorithm(&attributes, kGcm);
  psa_set_key_type(&attributes, PSA_KEY_TYPE_AES);
  psa_set_key_bits(&attributes, kAeadGcmKeySize * 8U);
  const psa_status_t result = psa_import_key(&attributes, key, kAeadGcmKeySize, &key_id);
...
bool seal(void* /*ctx*/, const std::uint8_t* key, const std::uint8_t* nonce,
  ...
  const psa_status_t result = psa_aead_encrypt(
      key_id, kGcm, nonce, kAeadGcmNonceSize, aad.size == 0 ? &empty : aad.data, aad.size,
      plaintext.size == 0 ? &empty : plaintext.data, plaintext.size, out,
      plaintext.size + kAeadGcmTagSize, &written);
  (void)psa_destroy_key(key_id);
  return result == PSA_SUCCESS && written == plaintext.size + kAeadGcmTagSize;
}
...
bool open(void* /*ctx*/, const std::uint8_t* key, const std::uint8_t* nonce,
  ...
  const std::size_t plaintext_size = ciphertext_and_tag.size - kAeadGcmTagSize;
  const std::uint8_t empty = 0;
  std::size_t written = 0;
  const psa_status_t result = psa_aead_decrypt(
      key_id, kGcm, nonce, kAeadGcmNonceSize, aad.size == 0 ? &empty : aad.data, aad.size,
      ciphertext_and_tag.data, ciphertext_and_tag.size, out, plaintext_size, &written);
  (void)psa_destroy_key(key_id);
...
const AeadGcm kPsaAeadGcm{&seal, &open, nullptr};

}  // namespace

const AeadGcm* psa_aead_gcm() noexcept { return &kPsaAeadGcm; }
