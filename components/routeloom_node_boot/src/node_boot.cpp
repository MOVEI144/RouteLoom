...
namespace {
// The app supplies its tag through NodeBootHooks::log_tag so a
// site mixing images can tell them apart in a captured log.
...
class NodeBootObserver final : public NodeObserver {
 public:
  void on_message(const MessageKey& key, const NodeId source,
    ...
    if (app_ != nullptr) app_->on_message(key, source, payload);
  }

  void on_group_message(const routeloom::GroupMessageInfo& info,
                        const ByteView payload) noexcept override {
    if (app_ != nullptr) app_->on_group_message(info, payload);
  }
  ...
  void on_delivery(const DeliveryResult& result) noexcept override {
    ...
    if (app_ != nullptr) app_->on_delivery(result);
  }
...
#endif
    if (app_ != nullptr) app_->on_diagnostic(reason, peer, message);
  }

  void bind_app(NodeObserver* app) noexcept { app_ = app; }
#if !CONFIG_ROUTELOOM_SECURITY_MODE_LEGACY_FIXTURE
...
class NodeBootConfigProvider final : public routeloom::ConfigProvider {
 public:
...
class NodeBootMaintenanceGate final : public routeloom::ConfigMaintenanceGate {
 public:
  explicit NodeBootMaintenanceGate(const bool independent_admin_path) noexcept
...
void run_node(const NodeBootHooks& hooks) {
  if (hooks.log_tag != nullptr) kTag = hooks.log_tag;
  ...
  static NodeBootObserver observer;
  observer.bind_app(hooks.observer);
#if !CONFIG_ROUTELOOM_SECURITY_MODE_LEGACY_FIXTURE
...
// The defined() guard is not redundant: this component compiles into every
// firmware project (bridge_node has ROUTELOOM_ROUTE_GATEWAY_SCOPED but no
// ROUTELOOM_ROUTE_GATEWAY_1 — its own main pins gateway[0] to NODE_ID).
  ...
  if (!status) fail(status.detail);
#if CONFIG_ROUTELOOM_SECURITY_MODE_LEGACY_FIXTURE
  if (hooks.attach != nullptr) hooks.attach(runtime, nullptr, &sdkv1_stores, hooks.ctx);
#endif
  ...
  static NodeBootConfigProvider config_provider;
  ...
  static NodeBootMaintenanceGate config_gate(/*independent_admin_path=*/false);
  ...
  // sends must run on the thread that drains events, not alongside a
  // runtime task.
