...
| Replay/counter epoch exhaustion | Wire v1 epochs are u16 (65,535 per peer context); floors compare ordinally, so wrapping to epoch 1 while a peer floor sits at 65,535 wedges that pair fail-closed. Modular comparison is rejected: it would re-admit a previously used epoch's key/nonce space | Security owner: wire v2 32-bit epoch namespace or an explicit exhaustion→re-provision lifecycle (aligned with persisted counter wear-out); until then, provisioned deployments must treat ~65,535 boots per peer pair as a re-provisioning bound |
