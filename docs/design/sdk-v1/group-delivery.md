...
**sleepとの関係**（#110）：上の表のgroup状態は全てRAMのみで、sleep imageには一切保存しない。drainは収集中のround（送信元木・中継木のreport待ち）が終わるまで待ち、queued originや予約済みrepairは待たずにdrain期限または完了後の精算へ回す。image commit成功後、未決着のoriginはpolicy別に明示終端する（Fail＝`Failed`/`SLEEP_DRAIN`、Save＝`Failed`/`SLEEP_GROUP_NOT_PERSISTED`、Defer＝`Indeterminate`/`SLEEP_DEFERRED`。いずれも`on_group_delivery`が出る）。受信側のORDERED holdも同じcommit後に、hold期限切れと同じ欠番skip経路で順序通りアプリへ放出し、`READY_TO_SLEEP`時点でholdは空になる。
