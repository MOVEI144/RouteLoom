...
対象コード：`components/routeloom/src/group.cpp`（配送engine・順序）、`group_wire.cpp`／`group.hpp`（payload codec）、`group_replay.cpp`／`group_replay.hpp`（group scopeのreplay表）、`wire.cpp`（`seal_group`／`open_group`）、`node.cpp`（配線、unicastの順序）、`c_api.cpp`／`routeloom.h`（C ABI）、`usb_host_ops.cpp`／`usb_bridge.cpp`（USB HostOps 0x50〜0x52）、`components/routeloom_espnow/src/psk_security.cpp`（開発PSKのgroup scope）。Rust：`host/routeloom-wire/src/group.rs`、`host/routeloom-protocol/src/group_ops.rs`、daemon lane `host/routeloom-host/src/group.rs`（API1は`api1.rs`）、`host/routeloom-client`（`send_group`／`group_result`）。試験：`tests/cpp/test_group.cpp`ほか（§12）。関連：[Wire](../../spec/wire-protocol.md)、[USB §8](../../spec/usb-protocol.md)、[無線§14](../../spec/radio.md)、[互換性](../../spec/compatibility.md)。
...
**sleepとの関係**（#110）：上の表のgroup状態は全てRAMのみで、sleep imageには一切保存しない。drainは収集中のround（送信元木・中継木のreport待ち）が終わるまで待ち、queued originや予約済みrepairは待たずにdrain期限または完了後の精算へ回す。repair roundは `kRetryRounds` として drain 中は開始せず（unicast のend-to-end再送と同じ扱い）、未完了分は Fail/Save/Defer で精算する。image commit成功後、未決着のoriginはpolicy別に明示終端する（Fail＝`Failed`/`SLEEP_DRAIN`、Save＝`Failed`/`SLEEP_GROUP_NOT_PERSISTED`、Defer＝`Indeterminate`/`SLEEP_DEFERRED`。いずれも`on_group_delivery`が出る）。受信側のORDERED holdも同じcommit後に、hold期限切れと同じ欠番skip経路で順序通りアプリへ放出し、`READY_TO_SLEEP`時点でholdは空になる。精算・ORDERED holdの強制放出はimage commit成功後、ticket発行前に行う。callback内のcoordinator操作は要求として記録し、精算1件またはhold本文1件が完了した安全点で取消しを反映する。abort受付直後のcallback内送信は `NODE_DRAINING` で拒否する。放出済み本文・skipは取り消さず、残りのholdは保持する。新prepareは次の外側pollで開始する。holdの粒度・複数source間の順序・gap/late規則は§5、power側の契約は[power.md §2.1](../../spec/power.md)を参照。
...
- **daemon API1（実装済み・host試験済み、EXPERIMENTAL）**：`group.send {network, group:1..65535|"ALL", key, payload_hex, payload_len, options:{priority, ordered, ttl_ms, hop_limit}, wait_ms?}`と`group.get {group_op, wait_ms?}`が同じgroup record（`group_op`、`state`、`final`、`result`、`reason`、`message:{session, sequence}`、`rounds`、`delivered`、`nonmember`、`missing_total`、`unaccounted`、`missing[]`、`missing_truncated`、時刻）を返し、終端で`events`に`group_settled`を1件出す。gatewayがcapability bit 7（とbit 2）を広告しなければ`UNSUPPORTED`、gatewayの拒否（flat profileの`GROUP_REQUIRES_GATEWAY_SCOPED`等）は`state:"REFUSED"`のrecord。daemonは0x51の即時応答とFINALを同じrequest idで受け、未決着の間は0x52で読み直し（FINAL喪失・再接続対策）、結果をRAMの有界表（256件）に置くので機器の回収後も読める。設計案からの変更：`group.get`はMessageIdではなくdaemonのop token（`grp…`）で引く（gatewayに届く前のHOST_QUEUEDにはMessageIdが無いため。MessageIdはrecordの`message`に載る）、payloadはAPI1の他methodに合わせhex、idempotency keyを必須にした。権限・エラー・上限は[host §10](../../spec/host.md)。アプリ向けには`routeloom-client`の`MeshTransport::send_group`／`group_result`（4操作契約は不変、既定実装はUNSUPPORTED）と`routeloomctl group-send`／`group-get`。
...
- **daemon API1**：host試験（lane状態機械、USB golden byte一致、daemon配線）のみで、実機のbridge_nodeとの疎通は未確認。group表はRAMのみ（daemon再起動で結果とidempotencyを失う）。membershipの遠隔設定はUSBにもAPI1にも無い。実RF・実機での到達時間・air timeは未測定。
