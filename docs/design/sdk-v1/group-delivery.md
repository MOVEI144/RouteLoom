...
**sleepとの関係**（#110）：上の表のgroup状態は全てRAMのみで、sleep imageには一切保存しない。drainは収集中のround（送信元木・中継木のreport待ち）が終わるまで待ち、queued originや予約済みrepairは待たずにdrain期限または完了後の精算へ回す。repair roundは `kRetryRounds` として drain 中は開始せず（unicast のend-to-end再送と同じ扱い）、未完了分は Fail/Save/Defer で精算する。image commit成功後、未決着のoriginはpolicy別に明示終端する（Fail＝`Failed`/`SLEEP_DRAIN`、Save＝`Failed`/`SLEEP_GROUP_NOT_PERSISTED`、Defer＝`Indeterminate`/`SLEEP_DEFERRED`。いずれも`on_group_delivery`が出る）。受信側のORDERED holdも同じcommit後に、hold期限切れと同じ欠番skip経路で順序通りアプリへ放出し、`READY_TO_SLEEP`時点でholdは空になる。精算・ORDERED holdの強制放出はimage commit成功後、ticket発行前に行う。callback内のcoordinator操作は `Busy` で拒否し、精算・放出を中断しない（abortは前か後にだけ入る）。callback内送信はdrain中 `NODE_DRAINING` で拒否する。holdの粒度・複数source間の順序・gap/late規則は§5、power側の契約は[power.md §2.1](../../spec/power.md)を参照。
