...
**sleepとの関係**（#110）：上の表のgroup状態は全てRAMのみで、sleep imageには一切保存しない。drainは収集中のround（送信元木・中継木のreport待ち）が終わるまで待ち、queued originや予約済みrepairは待たずにdrain期限または完了後の精算へ回す。repair roundは `kRetryRounds` として drain 中は開始せず（unicast のend-to-end再送と同じ扱い）、未完了分は Fail/Save/Defer で精算する。image commit成功後、未決着のoriginはpolicy別に明示終端する（Fail＝`Failed`/`SLEEP_DRAIN`、Save＝`Failed`/`SLEEP_GROUP_NOT_PERSISTED`、Defer＝`Indeterminate`/`SLEEP_DEFERRED`。いずれも`on_group_delivery`が出る）。受信側のORDERED holdも同じcommit後に、hold期限切れと同じ欠番skip経路で順序通りアプリへ放出し、`READY_TO_SLEEP`時点でholdは空になる。終端通知・hold放出は `READY_TO_SLEEP` 遷移とticket発行より前に実行され、callback内の `sleep_abort` はそのままRUNNING復帰として尊重される。callbackからの再入（abort・再 `sleep_prepare`・`notify_app_event`）はcoordinatorの試行世代で識別し、古い試行の精算・ticket発行・遷移は再開しない。
