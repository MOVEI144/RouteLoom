...
| **P5 — group鍵** | | | |
| P5-1 | authority channel（AuthorityEnvelope、USB 0x43〜0x45〔0x64〜0x67を推奨、02 §7.4〕）、GK保存・配布・更新・pull、GroupLink/GroupEnd、Member scope鍵をGKから導出（**PR3でHost側のGK保存・配布・更新・pull・`group_keys.*`を実装済み**。portable channelはPR1で実装済み、機器側はPR2、Host・USB結線はPR4） | P3-3, P4-3 | V1-K05〜K07, V1-K09, V1-K11 |
| P5-2 | broadcast経路広告（opt-in capability）。group delivery設計と同時にレビュー | P5-1 | V1-K08 |
| **P6 — 削除** | | | |
| P6-1 | RRS1の発行、Host側snapshot配布・再送・Applied ACKと`operations.get`の段階表示を実装済み（PR A、fake transport）。機器側のgossip・執行、RemovalNotice配信と実transport配線は後続 | P5-1 | V1-R01〜R07, V1-R09, V1-R10 |
