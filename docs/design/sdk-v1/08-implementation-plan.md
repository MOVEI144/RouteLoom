...
| P3-3 | `routeloom-host`のSite Authority service（store、EDHOC responder、台帳）、API1 `join.*`/`devices.discovered.*`/`members.*`/`site.status`、KGuard mock client（**このbranchで実装済み**：`--site-authority DIR`、pure RustのEDHOC responder `host/routeloom-edhoc`（RFC 9529 §3を両roleでbyte一致、libedhocとのmethod 0 join transcriptを両方向でbyte一致、[protocol/edhoc-interop](../../../protocol/edhoc-interop/README.md)）、SQLite台帳（hash chain、MemberCert・DAMS・RRS1・GK・発見済み・参加要求）、判定engine、API1と`membership.revoke`、`routeloom-client::site`の`SiteAdmin`と`KGuardMock`。USB 0x60〜0x63への結線、authority channelのHost結線・USB搬送（P5 PR4）、RRS1配布のHost側（P6-1 PR A：snapshot・再送・Applied ACK・`operations.get`の進捗、fake transport）は実装済み。実transport配線・機器側執行は未実装。PR1のportable channelは実装済み。GKの保存・配布・更新・pullのHost側はP5 PR3で実装済み（`group_keys.*`、24時間・削除時更新、transportはfake）。証明書値渡しのEAD label 65541（P3-1で凍結）とDAMSのExporter context（P3-4で凍結）。機器側EDHOC arenaは2048Bに引き上げ済み（[07 §2.4](07-host-api-tooling.md)）） | P2-1, P3-2 | V1-J03, V1-J09, V1-H01〜H07 |
...
| **P5 — group鍵** | | | |
| P5-1 | authority channel（AuthorityEnvelope、USB 0x64〜0x67）、GK保存・配布・更新・pull、GroupLink/GroupEnd、Member scope鍵をGKから導出（**PR 1でportable endpoint codec／有界client、Rust responder／鍵導出、USB HostOps codecと共通vectorを実装・host試験済み**。PR 3でHost側のGK保存・配布・更新・pull・`group_keys.*`を実装済み。機器側はPR 2、実搬送とSite Authorityへの配線はPR 4） | P3-3, P4-3 | V1-K05〜K07, V1-K09, V1-K11 |
| P5-2 | broadcast経路広告（opt-in capability）。group delivery設計と同時にレビュー | P5-1 | V1-K08 |
| **P6 — 削除** | | | |
| P6-1 | RRS1の発行・gossip・執行、RemovalNotice、`membership.revoke`と段階表示（**PR Aでportable lifecycle／gossip／peer enforcementとRust配布状態・APIを実装・host試験済み**。Host側snapshot配布・再送・Applied ACKと`operations.get`の段階表示はfake transportで実装済み。RemovalNotice配信と実搬送への接続は後続） | P5-1 | V1-R01〜R07, V1-R09, V1-R10 |
