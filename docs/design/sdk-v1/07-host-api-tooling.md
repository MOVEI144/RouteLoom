...
| `join.policy.get` / `.set` | ADMIN | `zero_touch_open`、`decision_mode`（`kguard`/`closed`/`lab_inventory`）、`decision_timeout_ms`（500〜5000）、`pending_retry_after_s`（30〜3600）。setは部分更新。内容が変わるsetは`policy_generation`を+1（同内容の再送は据え置き）。get/set応答は`policy_generation`と`radio_distributed_generation`（proxyが適用確認した最新世代。無線配布の搬送が無いため現状は常にnull）も返す |
...
{"operation_id":"op-0000000000000002","state":"committed","device_id":"00a1000000001234",
 "generation":1,"rs_epoch":1,"gk_rotation":{"from":1,"to":2,"state":"committed"},
...
{"operation_id":"op-0000000000000002","kind":"revoke","device_id":"00a1000000001234","generation":1,
 ...
 "gk_rotation":{"from":1,"to":2,"state":"distributing"},"created_ms":1790000000030}
// group_keys.rotate → {"operation_id":"op-…","state":"committed","from":1,"to":2,"cause":"manual","targets":n}
// operations.get op-…（rotate）は state が committed→distributing→activated→converged
//（またはactivated_with_unknown。supersedeされた旧operationはsuperseded）
// group_keys.status
{"active":1,"staged":2,"phase":"staging","cause":"manual","targets":2,"staged_ack":1,
 "active_ack":0,"unknown":1,"last_rotation":null,"next_due_ms":null,
 "clock":{"unix_ms":…,"mono_ms":…}}
...
**開発siteの追加（D01）**：`lab-site-init` は site ごとに Device CA／Site CA／SAK と purpose manifest を生成し、daemon の初回 open で公開鍵 fingerprint と purpose を site.db に固定する。`lab_inventory` は development site の明示承認から単調時計で最大1時間だけ有効（再起動時は閉じる）。provision-confirm-written 済みの office ledger と、その出力先に残した DevCert の署名・NodeId・kid・serial・digest を `lab-inventory-import` で照合し、daemon を再起動した後だけ、認証済み JoinRequest の要求 role と照合して同じ `join.decide` で allow する。actor・policy/version・inventory revision・request attempt・operation は同じ DB transaction の `approval_audit` に永続化する。import/production site は自動承認不可（site.status の purpose は認証profile表示ではない）。
...
| 同じNodeIdで別kid（有効なmembership） | `join.request`に`kid_conflict:true`。自動allowしない。revoke後もそのNodeIdは再allow不可（履歴台帳から判定）。新NodeIdで再provisionする |
