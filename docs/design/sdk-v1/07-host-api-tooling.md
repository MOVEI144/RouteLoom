...
**開発siteの追加（D01）**：`lab-site-init` は site ごとに Device CA／Site CA／SAK を生成し、purpose と公開鍵 fingerprint を site.db に固定する。`lab_inventory` は development site の明示承認から単調時計で最大1時間だけ有効（再起動時は閉じる）。provision-confirm-written 済みの office ledger の (site Device CA, NodeId, kid) を `lab-inventory-import` して daemon を再起動した後だけ、認証済み JoinRequest の要求 role と照合し、同じ `join.decide` で allow する。actor・policy/version・inventory revision・request attempt・operation は同じ DB transaction の `approval_audit` に永続化する。import/production site は自動承認不可（site.status の purpose は認証profile表示ではない）。
