...
**P5-2 payload codec（未配線）**：`route_scale.cpp` と Rust `broadcast_route.rs` は version 1 の24B/record（destination、generation、sequence、metric、via）を最大5件まで厳格に検査する。受信者が `via` と一致したrecordだけ∞に射影できる。Wireの厳格なbroadcast RouteUpdate header gate、GroupLinkのboot/GK同時snapshotと専用open（明示指定時だけ）まで追加済み。通常のMeshNode受信はこの専用openを指定しない。送信・受信dispatch、capability交渉が未配線なのでbroadcast広告は有効ではない。ESP-NOW Ownerの予約済み送信slotからbroadcast MACへ送れるようになったが、経路広告のschedulerは未配線であり送信完了は受信者の到達証拠ではない。C++ `NodeConfig::route_broadcast` は既定false。trueはflat全dumpを `Unsupported` で拒否し、scopedでも安全な送受信が未配線の間は `Unsupported` で起動拒否する。C ABI とfirmwareにopt-inを公開しない。Diagnostic bit5をroute broadcast用に予約し、RRS gossipはbit7へ移した。Rust側ではDiagnostic CapabilitiesQuery/Reply本文（24/40 B）のstrict codecをC++と同じnonce/boot/feature/期限境界で検査するが、これは経路広告の送受信許可ではない。旧P6実験版のbit5 grantと区別するversionが無いため、旧版との相互運用が解決するまでbroadcast opt-inは起動拒否のままにする。旧版RRS bit5は新規gossip許可としては使わない。
