...
| join用EAD codec（P2-3）：JoinIntent／SiteOffer／JoinRequest／JoinResult／SitePackage、RemovalNotice | [sdkv1_ead.hpp](../../../components/routeloom/include/routeloom/sdkv1_ead.hpp)、Rust `host/routeloom-join` | 独立Python生成器の[共通vector](../../../protocol/sdkv1-golden/ead/README.md)（valid 35・invalid 147、V1-J12のAllow不一致とP3-1のCredential項目〔label 65541〕を含む）をC++（検証）とRust（Site AuthorityとしてMemberCert・RemovalNoticeをRFC 6979で再発行）がbyte一致で通過、`fuzz_sdkv1_ead`。EAD labelと未定点は[02 §6.3](02-zero-touch-join.md)、kid参照＋証明書EADは[02 §3](02-zero-touch-join.md) |
| 参加の搬送（P3-1）と中継（P3-2）：RLD1 body v3、BootstrapAuth phase 4〜6、1024B組立object、relay object、`ZtJoinerLink`／`JoinProxy`／`JoinRelayGateway`、USB HostOps 0x60〜0x63（capability bit 8） | [sdkv1_join_transport.hpp](../../../components/routeloom/include/routeloom/sdkv1_join_transport.hpp)、[sdkv1_join_relay.hpp](../../../components/routeloom/include/routeloom/sdkv1_join_relay.hpp)、[usb_host_ops.hpp](../../../components/routeloom/include/routeloom/usb_host_ops.hpp)、Rust `routeloom-protocol::join_relay` | 独立Python生成器の[共通vector](../../../protocol/sdkv1-golden/join-transport/README.md)（valid 52・invalid 103）とUSB session vector（`protocol/usb-golden/join-relay`）をC++とRustが通過、`fuzz_sdkv1_join`、device→proxy→gateway→authorityのhost模擬（V1-J02・J10・J11〔proxy側〕・H08）、EAD込みEDHOC交換の実長（V1-J14）。firmware・MeshNode未配線（[02 §5.4・§7.4](02-zero-touch-join.md)） |
...
| Site Authority service（P3-3） | `routeloom-host`の[site/](../../../host/routeloom-host/src/site/mod.rs)（`--site-authority DIR`）、pure RustのEDHOC Responder `host/routeloom-edhoc`、`routeloom-client::site`（`SiteAdmin`、`KGuardMock`） | RFC 9529 §3を両roleでbyte一致、libedhocとのmethod 0参加transcriptを両方向でbyte一致（[protocol/edhoc-interop](../../../protocol/edhoc-interop/README.md)）、判定engine・SQLite台帳・API1・削除（RRS1／RemovalNotice）をhost試験、daemonのAPI1 socket越しにKGuard mockで端から端まで。daemonのUSB中継（0x60〜0x63、codecはP3-2）は未結線、GK配布・authority channelはP5（[07 §2.4](07-host-api-tooling.md)） |
それ以外（EDHOCのNode／参加FSMへの統合・C3/S3実測、参加搬送のfirmware・MeshNode配線、RLRES1のNode・carrier配線、group鍵の配布・更新、参加のUSB中継のdaemon結線（USB codecはP3-2、API1のSite Authority面はP3-3で実装）、tooling、partition変更）は**未実装**。
