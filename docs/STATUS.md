...
| 経路制御 | feasibility、withdraw、SeqNoRequest、generation／tombstone／hold-down、bounded seqno、3hop／diamond repairに加え、10hop配送、分断再結合、loop-freedomをportable model testで実装・確認。100台向けgateway-scoped profile（issue #41、[設計](design/sdk-v1/routing-scale.md)：gateway木の周期更新・上り部分木・ROUTE_REQUESTによるpull／on-demand探索、lease規則をconfig検査で強制）をportable coreに実装し、100台SimWorld（5s tick／90s lease、4分間gateway往復到達の途切れ0・loop 0・推定制御airtime 78ms/s）でhost試験済み。`NodeConfig::route_gateways`を未設定なら従来のflat profile。C API／firmware設定への配線と実RFでの認定は未実施 |
